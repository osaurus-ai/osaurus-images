...
```json
{
  "plugin_id": "dev.example.osaurus-images",
  "name": "Osaurus Images",
  "version": "0.1.0",
  "secrets": [
    {
...
```bash
# Package the plugin
osaurus tools package dev.example.osaurus-images 0.1.0

# Install locally
osaurus tools install ./dev.example.osaurus-images-0.1.0.zip
...
```bash
swift build -c release && osaurus tools package dev.example.osaurus-images 0.1.0 && osaurus tools install ./dev.example.osaurus-images-0.1.0.zip
```
...
codesign --force --options runtime --timestamp \
  ...
  .build/release/libosaurus-images.dylib
```
